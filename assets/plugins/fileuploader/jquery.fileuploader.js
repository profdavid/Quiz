...
(function($) {
	...
	$.fn.fileuploader.defaults = {
        ...
        captions: {
            button: function(options) { return (options.limit == 1 ? 'Escolher' : 'Escolher Arquivos'); },
            feedback: function(options) { return (options.limit == 1 ? 'Arquivo' : 'Arquivos') + ' para upload'; },
            feedback2: function(options) { return options.length + ' ' + (options.length > 1 ? ' arquivos foram escolhidos' : ' arquivo foi escolhido'); },
            drop: 'Arraste os arquivos para Upload',
            paste: '<div class="fileuploader-pending-loader"><div class="left-half" style="animation-duration: ${ms}s"></div><div class="spinner" style="animation-duration: ${ms}s"></div><div class="right-half" style="animation-duration: ${ms}s"></div></div> Pasting a file, click here to cancel.',
            removeConfirmation: 'Deseja remover este arquivo?',
            errors: {
                filesLimit: 'Apenas ${limit} é permitido.',
                filesType: 'Apenas as extensões ${extensions} são permitidas.',
                fileSize: '${name} é muito grande! Escolha um arquivo até ${fileMaxSize} MB.',
                filesSizeAll: 'Os arquivos escolhidos são muito grandes! Tamanho total máximo ${maxSize} MB.',
                fileName: 'O arquivo de nome ${name} já foi selecionado.',
                folderUpload: 'Você não pode fazer upload de pastas.'
            }
        }
	}
})(jQuery);